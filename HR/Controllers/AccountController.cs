...
namespace HR.Controllers
{
   ...
   public class AccountController : ControllerBase
   {
                     ...
                     if (profile.User_Status.Equals("Active"))
                     {
                        ...
                        else
                        {
                           ...
                           else
                           {
                              ...
                              var empHist = hService.GetCurrentEmploymentHistoryByProfile(profile.Profile_ID);
      ...
      //        {
      //            profile = userService.getUserProfileByUserName(model.Email);
      //            if (profile != null && profile.Is_Email.HasValue)
      //                user = UserManager.FindById(profile.User_Authentication.ApplicationUser_Id);
      //        }
      //        else
      //        {
      ...
      public ActionResult ForgotPassword(string email)
      {
         if (!string.IsNullOrEmpty(email))
         {
            ...
            if (email.IndexOf('@') < 0)
            {
               user = uService.getUserProfileByUserName(email);
               IsEmail = false;
            }
            else
            {
            ...
            if (user != null)
            {
               if (user.User_Authentication != null && ((user.User_Authentication.Is_Email.HasValue ? user.User_Authentication.Is_Email.Value : true) == IsEmail))
               {
      ...
      public ActionResult SaveSkipTutorial(bool IsSkip)
      {
         if (IsSkip == true)
         {
            var uService = new UserService();
            var userlogin = UserSession.getUser(HttpContext);
            if (userlogin != null) uService.SaveSkipTutorial(userlogin.Profile_ID, IsSkip);
         }
         return Json(JsonRequestBehavior.AllowGet);
      }
