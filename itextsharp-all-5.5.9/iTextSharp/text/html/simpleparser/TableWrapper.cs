...
namespace iTextSharp.text.html.simpleparser
{
   ...
   public class TableWrapper : IElement
   {
      ...
      virtual public PdfPTable CreateTable()
      {
         ...
         string repeatheader;
         styles.TryGetValue(HtmlTags.REPEATHEADER, out repeatheader);
         if (repeatheader != null)
            table.HeaderRows = Int32.Parse(repeatheader);

         table.SplitRows = true;
         string splitrow;
         styles.TryGetValue(HtmlTags.SPLITROW, out splitrow);
         if (splitrow != null)
            table.SplitRows = Boolean.Parse(splitrow);

         //table.SplitLate = true;
         string splitlate;
         styles.TryGetValue(HtmlTags.SPLITLATE, out splitlate);
         if (splitlate != null)
            table.SplitLate = Boolean.Parse(splitlate);
         ...
         String paddingtop;
         styles.TryGetValue(HtmlTags.PADDINGTOP, out paddingtop);
         String paddingleft;
         styles.TryGetValue(HtmlTags.PADDINGLEFT, out paddingleft);
         String paddingbuttom;
         styles.TryGetValue(HtmlTags.PADDINGBUTTOM, out paddingbuttom);
         String paddingright;
         styles.TryGetValue(HtmlTags.PADDINGRIGHT, out paddingright);
         String padding;
         styles.TryGetValue(HtmlTags.PADDING, out padding);
         String rowpadding;
         styles.TryGetValue(HtmlTags.ROWPADDING, out rowpadding);
         string useascender;
         styles.TryGetValue(HtmlTags.USEASCENDER, out useascender);
         // column widths
         try
         {
            if (colWidths != null)
         ...
         foreach (IList<PdfPCell> col in rows)
         {
            ...
            foreach (PdfPCell pc in col)
            {
               ...
               if (padding != null)
               {
                  ...
                  {
                     pc.UseAscender = true;
                     pc.PaddingTop = float.Parse(padding);
                  }
                  if (r == rows.Count - 1)
                  {
                     if (rowpadding == null)
                     {
                        pc.PaddingBottom = float.Parse(padding);
                     }
                  }
                  if (c == 0)
                     pc.PaddingLeft = float.Parse(padding);
                  if (c == col.Count - 1)
                     pc.PaddingRight = float.Parse(padding);
               }
               else
               {
                  if (paddingtop != null)
                  {
                     ...
                     {
                        pc.UseAscender = true;
                        pc.PaddingTop = float.Parse(paddingtop);
                     }
                  }
                  if (paddingbuttom != null)
                  {
                     if (r == rows.Count - 1)
                        if (rowpadding == null)
                           pc.PaddingBottom = float.Parse(paddingbuttom);
                  }
                  if (paddingleft != null)
                  {
                     if (c == 0)
                        pc.PaddingLeft = float.Parse(paddingleft);
                  }
                  if (paddingright != null)
                  {
                     if (c == col.Count - 1)
                        pc.PaddingRight = float.Parse(paddingright);
                  }
               }

               if (rowpadding != null)
               {
                  var introwpadding = Convert.ToInt32( rowpadding);
                  pc.PaddingTop += float.Parse(rowpadding);
                  pc.PaddingBottom += float.Parse((introwpadding +2).ToString());
               }
               table.AddCell(pc);
               c++;
            }
