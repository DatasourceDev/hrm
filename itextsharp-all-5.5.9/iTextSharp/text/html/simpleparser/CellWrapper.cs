...
namespace iTextSharp.text.html.simpleparser
{
   ...
   public class CellWrapper : ITextElementArray
   {
      ...
      public class RoundRectangle : IPdfPCellEvent
      {
         ...
         private BaseColor _color { get; set; }
         public RoundRectangle()
         {

         }
         public RoundRectangle(float radius, BaseColor color)
         {
            _radius = radius;
            _color = color;
         }
         public void CellLayout(PdfPCell cell, Rectangle rect, PdfContentByte[] canvas)
         {
            ...
            cb.RoundRectangle(
              ...
              rect.Width,
              rect.Height,
              _radius // change to adjust how "round" corner is displayed
            );
            cb.SetLineWidth(1f);
            cb.SetColorStroke(_color);
            cb.SetColorFill(_color);           
            cb.Stroke();
         }
      }
      ...
      virtual public PdfPCell CreatePdfPCell(String tag, ChainedProperties chain)
      {
         ...
         value = chain[HtmlTags.BORDERRADIUS];
         if (value != null)
         {
            float radius = float.Parse(value);
            value = chain[HtmlTags.BORDERCOLOR];
            if (value != null)
               cell.CellEvent = new RoundRectangle(radius, HtmlUtilities.DecodeColor(value));
         }
          value = chain[HtmlTags.BORDERCOLOR];
          if (value != null)
          {
